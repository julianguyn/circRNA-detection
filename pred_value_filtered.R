...
mysheets <- read_excel_allsheets("../data/biomarker/1-s2.0-S2405471220303239-mmc3.xlsx")

# filtered for only cell lines classified as matching
GDSC2 <- as.data.frame(mysheets[1])
CCLE <- as.data.frame(mysheets[2])
gCSI <- as.data.frame(mysheets[3])

colnames(GDSC2) <- GDSC2[1,]
colnames(CCLE) <- CCLE[1,]
colnames(gCSI) <- gCSI[1,]

GDSC2_filtered <- GDSC2[which(GDSC2$baf.p.fit == "M"),]
CCLE_filtered <- CCLE[which(CCLE$baf.p.fit == "M"),]
gCSI_filtered <- gCSI[which(gCSI$baf.p.fit == "M"),]


# load in biological replicates
load("../results/inter.RData")

# filter to only include cell lines clssified as matching
replicates_filtered <- intersected_rnacells[which(intersected_rnacells %in% gCSI_filtered$RNAseq_Cell_ID)]
replicates_filtered <- replicates_filtered[which(replicates_filtered %in% CCLE_filtered$RNAseq_Cell_ID)]
replicates_filtered <- replicates_filtered[which(replicates_filtered %in% GDSC2_filtered$RNAseq_Cell_ID)]


##################################################
##### Repeat analysis from hist_pred_value.R #####
##################################################

#read in PSets isoforms.counts
load("../results/Kallisto_isoforms_counts.RData")

# keep replicates classified as Matching
gCSI_rna <- gCSI_rna@assays@data$expr[,which(colnames(gCSI_rna@assays@data$expr) %in% replicates_filtered)]
CCLE_rna <- CCLE_rna@assays@data$expr[,which(colnames(CCLE_rna@assays@data$expr) %in% replicates_filtered)]
GDSC_rna <- GDSC_rna@assays@data$expr[,which(colnames(GDSC_rna@assays@data$expr) %in% replicates_filtered)]
...
#gcsi/ccle
gcsi_ccle_stability <- as.data.frame(sapply(transcripts, function(i) cor(as.numeric(gCSI_rna[i,]),as.numeric(CCLE_rna[i,]), method="spearman")))

#gcsi/gdsc
gcsi_gdsc_stability <- as.data.frame(sapply(transcripts, function(i) cor(as.numeric(gCSI_rna[i,]),as.numeric(GDSC_rna[i,]), method="spearman")))

#gdsc/ccle
gdsc_ccle_stability <- as.data.frame(sapply(transcripts, function(i) cor(as.numeric(GDSC_rna[i,]),as.numeric(CCLE_rna[i,]), method="spearman")))
...
genes <- gCSI@molecularProfiles$Kallisto_0.46.1.isoforms.counts@elementMetadata$gene_name[match(rownames(meta_pearson), gCSI@molecularProfiles$Kallisto_0.46.1.isoforms.counts@elementMetadata$transcript_id)]
...
save(spearman_df, spearman_df_randomize, file = "../results/pred_value_filtered.RData")

# plot histograms
spearman_df$gene <- rownames(spearman_df)
spearman_df <- melt(spearman_df)
spearman_df$label <- c(rep("Non-randomized SI", 1148))

spearman_df_randomize$gene <- rownames(spearman_df_randomize)
spearman_df_randomize <- melt(spearman_df_randomize)
spearman_df_randomize$label <- c(rep("Randomized SI", 1148))
